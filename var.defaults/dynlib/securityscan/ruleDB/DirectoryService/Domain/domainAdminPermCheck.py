...
class RuleDictResult(DictResult):
    ...
    def getStatus(self):
        ...
        resp = execWebAPI("SYNO.Core.Directory.Domain.Conf", "get", 1)
        if not resp or not resp["success"]:
            return SZ_ERROR
        if resp["data"]["disable_domain_admins"]:
            return SZ_PASS
        self._extraData = "fail"
        return SZ_FAIL

    def getMethod(self):
        return {METHOD_ACTION: METHOD_ACTION_LINK, METHOD_ACTION_VAL: 'SYNO.SDS.AdminCenter.Application:SYNO.SDS.AdminCenter.DirectoryService.Main:domain'}
    ...
    def getAction(self):
        if "" != self._extraData:
            ...
            action[ACTION_REPLACE_VAR]['%1'] = "_T('domain', 'disable_admin')"
