...
#ifdef SYNOSMB_AD_SERVER
def syno_shlex_split(data):
    '''split data into list and return list

    About data format:
    data is sigle string format or multiple string format(RFC4408).
    Please note data conforms RFC4408, this means each part in data
    shall be enclosed by '"'. So that content of each part shall not
    contains '"', unless '"' is escaped by '\' it can contains double
    qoute(i.e. '\"'). Besides '\' is used for escaped, so that if you
    want to input '\', you need escaped '\' by '\'.(i.e. \\).

    About return format:
    return value is a list which each item is a part of data which
    enclosed by '"'.

    e.g.
    >>> data=r'"123"'
    >>> syno_shlex_split(data)
    ['123']
    >>> data=r'123'
    >>> syno_shlex_split(data)
    ['123']
    ...
    NOTE: This function replaced origin shlex.split() in this file,
    by shlex.split() to split data the \ will be eaten, i.e. \\
    becomes \, \" becomes ", i.e. if user input "\"" content of TXT for
    create/edit record, after parsing this content by shlex.split().
    "\"" will become """.

    This brings some issue for Synology DNS Server for processing TXT
    records:
        1. Save """ in samba dns DB is hard to process(when read, need to
    add \ back)
        2. The eaten \ format not conforms to file base TXT content format
    of bind-9.x.

    So for the consistent with file based TXT content format of bind-9.x
    and for easy processing that we need our own syno_shlex_split() to
    avoid \ to be eaten. After this modification, 'samba-tool dns' list
    records for TXT conforms with file base format of bind-9.x. But pure
    '"' of a part in samba dns DB is not allowed any more.
    '''
    import re
    regex = r'"((([\\].)|[^"\\]*)*)"' # This is element pattern of multiple string
    retArr = re.findall(regex, data)
    if retArr:
        # NOTE that the 'double qoute' in the begin and end 'REMOVED'
        retArr = [item[0] for item in retArr] # multiple string case(each part wrapped in "(double quote))
    else:
        # NOTE that the 'double qoute' in the begin and end 'NOT REMOVED'
        retArr = [data] # single string case(string not wrap in "(double quote))
    return retArr

class cmd_synoquery(Command):
    """Query a name."""
    ...
    def enum_record(self, dns_conn, dummy0, dummy1, server, zone, name, *args):
        buflen, this_name_records = dns_conn.DnssrvEnumRecords2(dummy0, dummy1, server, zone, name, *args)
        for rec in this_name_records.rec:
            if "" == rec.dnsNodeName.str:
                self.outf.write('  Name=%s, Records=%d, Children=%d\n' % (
                            name,
                            rec.wRecordCount,
                            rec.dwChildCount))
                for dns_rec in rec.records:
                        print_dns_record(self.outf, dns_rec)
            else:
                try:
                    self.enum_record(dns_conn, dummy0, dummy1, server, zone, rec.dnsNodeName.str +"." + name, *args)
                except RuntimeError as (enum, estr):
                    if enum == 0x000025F2: #WERR_DNS_ERROR_NAME_DOES_NOT_EXIST
                        continue
                    else:
                        raise CommandError("lookup of server=%s zone=%s name=%s failed: %s" % (server, zone, name, estr))
    ...
    def run(self, server, zone, name, rtype, authority=False, cache=False,
        ...
        ''' ********************* NOTE **************************
            Above code is non-modified, just add a enum_record function to replace the following ccode:
            "
            buflen, res = dns_conn.DnssrvEnumRecords2(
...
class cmd_dns(SuperCommand):
    ...
    #ifdef SYNOSMB_AD_SERVER
    subcommands['synoquery'] = cmd_synoquery()
    #endif
