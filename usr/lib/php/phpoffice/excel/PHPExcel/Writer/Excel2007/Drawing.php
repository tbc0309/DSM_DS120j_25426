...
class PHPExcel_Writer_Excel2007_Drawing extends PHPExcel_Writer_Excel2007_WriterPart
{
    ...
    public function writeChart(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Chart $pChart = null, $pRelationId = -1)
    {
        ...
        /***SYNO start***/
        if ('absoluteAnchor' === $pChart->getAnchorType()) {
            // xdr:absoluteAnchor
            $objWriter->startElement('xdr:absoluteAnchor');

            $objWriter->startElement('xdr:pos');
            $objWriter->writeAttribute('x', PHPExcel_Shared_Drawing::pixelsToEMU($pChart->getTopLeftXOffset()));
            $objWriter->writeAttribute('y', PHPExcel_Shared_Drawing::pixelsToEMU($pChart->getTopLeftYOffset()));
            $objWriter->endElement();
            // xdr:ext
            $objWriter->startElement('xdr:ext');
            $objWriter->writeAttribute('cx', PHPExcel_Shared_Drawing::pixelsToEMU($pChart->getBottomRightXOffset() - $pChart->getTopLeftXOffset()));
            $objWriter->writeAttribute('cy', PHPExcel_Shared_Drawing::pixelsToEMU($pChart->getBottomRightYOffset() - $pChart->getTopLeftYOffset()));
            $objWriter->endElement();
        } else  {
        /***SYNO end***/
    ...
    public function writeDrawing(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet_BaseDrawing $pDrawing = null, $pRelationId = -1)
    {
        if ($pRelationId >= 0) {
        /***SYNO start***/
        if ('absoluteAnchor' === $pDrawing->getAnchorType()) {
            // xdr:absoluteAnchor
            $objWriter->startElement('xdr:absoluteAnchor');

            // xdr:ext
            $objWriter->startElement('xdr:pos');
            $objWriter->writeAttribute('x', PHPExcel_Shared_Drawing::pixelsToEMU($pDrawing->getOffsetX()));
            $objWriter->writeAttribute('y', PHPExcel_Shared_Drawing::pixelsToEMU($pDrawing->getOffsetY()));
            $objWriter->endElement();
        } else  {
        /***SYNO end***/
            // xdr:oneCellAnchor
