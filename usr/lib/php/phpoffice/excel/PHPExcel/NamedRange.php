...
class PHPExcel_NamedRange
{
    ...
    /*** SYNO start ***/
    // Workaround, only used when scope/worksheet is not loaded
    private $localSheetId;

    // Raw defined name value
    private $rawValue;
    ...
    {
        /*** SYNO start ***/
        // Force scope to PHPExcel_Worksheet when called
        if (gettype($this->scope) === 'string') {
            $this->scope = $this->getWorksheet()->getParent()->getSheetByName($this->scope);
        }
        /*** SYNO end ***/
        return $this->scope;
    }
    ...
    {
        if (gettype($this->scope) === 'object') {
            return $this->scope->getTitle();
        }
        return $this->scope;
    }

    public function setLocalSheetId($localSheetId)
    {
        $this->localSheetId = $localSheetId;
    }

    public function getLocalSheetId()
    {
        if (gettype($this->scope) === 'object') {
            $this->localSheetId = $this->scope->getParent()->getIndex($this->scope);
        }
