        ...
        foreach ($rels->Relationship as $rel) {
            if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
                ...
                foreach ($relsWorkbook->Relationship as $ele) {
                    ...
                    /*** SYNO start ***/
                    else if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/chartsheet") {
                        $worksheets[(string) $ele["Id"]] = $ele["Target"];
                    }
                    /*** SYNO end ***/
                }
...
//                echo 'Adjusted Formula is ', $value, PHP_EOL;
    ...
    public function load($pFilename)
    {
        ...
        foreach ($rels->Relationship as $rel) {
            ...
            if ($xmlStyles->colors && $xmlStyles->colors->indexedColors && $xmlStyles->colors->indexedColors->rgbColor) {
                foreach ($xmlStyles->colors->indexedColors->rgbColor as $rgbColor) {
                $colors[] = self::readColor($rgbColor);
                }
            }
            }
            if (!empty($colors)) {
                PHPExcel_Style_Color::setIndexedColor($colors);
            }
            /***SYNO end***/
                ...
                /***SYNO start***/
                if (($numFmt === PHPExcel_Style_NumberFormat::FORMAT_GENERAL || empty($numFmt)) &&
                /***SYNO end***/
                                   (int)$xf["numFmtId"] < 164 && PHPExcel_Style_NumberFormat::builtInFormatCode((int)$xf["numFmtId"]) !== '') {
                        ...
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            if (isset($xmlSheet->sheetViews) && isset($xmlSheet->sheetViews->sheetView)) {
                                ...
                                if (isset($xmlSheet->sheetViews->sheetView->pane)) {
                                    ...
                                    if (isset($xmlSheet->sheetViews->sheetView->pane['xSplit'])) {
                                       $xSplit = intval($xmlSheet->sheetViews->sheetView->pane['xSplit']);
                                    }

                                    if (isset($xmlSheet->sheetViews->sheetView->pane['ySplit'])) {
                                        $ySplit = intval($xmlSheet->sheetViews->sheetView->pane['ySplit']);
                                    }
                                    if (isset($xmlSheet->sheetViews->sheetView->pane['state'])) {
                                        $state = (string) $xmlSheet->sheetViews->sheetView->pane['state'];
                                    }
                                    if (($xSplit > 0 || $ySplit > 0) && ($state === 'frozen' || $state === 'frozenSplit')) {
                                        $docSheet->freezePane(PHPExcel_Cell::stringFromColumnIndex($xSplit) . ($ySplit + 1));
                                    }
                                    /***SYNO End***/
                                }
                            ...
                            if (isset($xmlSheet->cols) && !$this->readDataOnly) {
                                foreach ($xmlSheet->cols->col as $col) {
                                    /* SYNO start */
                                    if (PHPExcel_Worksheet_ColumnDimension::isGlobalMode() && intval($col["max"]) === 16384 && $col["style"]) {
                                        $docSheet->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex(intval($col["min"])))->
                                            setXfIndex(intval($col["style"]))->setMax(16384);
                                        break;
                                    }
                                    /* SYNO start */
    ...
    //                                    echo 'Value is ', $value, PHP_EOL;
                            ...
                            if (!$this->readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
                                ...
                                foreach ($xmlSheet->conditionalFormatting as $conditional) {
                                    foreach ($conditional->cfRule as $cfRule) {
                                        if (in_array((string)$cfRule["type"], $supportedType) && isset($dxfs[intval($cfRule["dxfId"])])) {
                                            ...
                                            $objConditional->setPriority(intval($cfRule["priority"]));

                                            $sqref = (string)$conditional["sqref"];
                                            $conditionals[$sqref][] = $objConditional;
                                        }
                                    }
                                }

                                foreach ($conditionals as $sqref => $objConditionals) {
                                    $docSheet->setConditionalStyles($sqref, $objConditionals);
                                }
                                /*** SYNO end ***/
                            ...
                            if ($xmlSheet && $xmlSheet->dataValidations && !$this->readDataOnly) {
                                foreach ($xmlSheet->dataValidations->dataValidation as $dataValidation) {
                                    ...
                                    /*** SYNO start ***/
                                    $docValidation = new PHPExcel_Cell_DataValidation();
                                    $docValidation->setType((string) $dataValidation["type"]);
                                    if (!is_null($dataValidation["errorStyle"])) {
                                        $docValidation->setErrorStyle((string) $dataValidation["errorStyle"]);
                                    }
                                    $docValidation->setOperator((string) $dataValidation["operator"]);
                                    $docValidation->setAllowBlank(self::boolean((string) $dataValidation["allowBlank"]));
                                    $docValidation->setShowDropDown(!self::boolean((string) $dataValidation["showDropDown"]));
                                    $docValidation->setShowInputMessage(self::boolean((string) $dataValidation["showInputMessage"]));
                                    $docValidation->setShowErrorMessage(self::boolean((string) $dataValidation["showErrorMessage"]));
                                    ...
                                    $docSheet->setDataValidation($range, $docValidation);
                                    /*** SYNO end ***/
                                }
                            }

                            /*** SYNO start ***/
                            if ($xmlSheet && !$this->readDataOnly && $xmlSheet->extLst && $xmlSheet->extLst->ext) {
                                $ext = $xmlSheet->extLst->ext->children('x14', true);
                                if ($ext->dataValidations) {
                                    foreach ($ext->dataValidations->dataValidation as $dataValidation) {
                                        $range = strtoupper($dataValidation->children('xm', true)->sqref);
                                        $attrs = $dataValidation->attributes();
                                        $docValidation = new PHPExcel_Cell_DataValidation();
                                        $docValidation->setType((string) $attrs["type"]);
                                        if (!is_null($attrs["errorStyle"])) {
                                            $docValidation->setErrorStyle((string) $attrs["errorStyle"]);
                                        }
                                        $docValidation->setOperator((string) $attrs["operator"]);
                                        $docValidation->setAllowBlank(self::boolean((string) $attrs["allowBlank"]));
                                        $docValidation->setShowDropDown(!self::boolean((string) $attrs["showDropDown"]));
                                        $docValidation->setShowInputMessage(self::boolean((string) $attrs["showInputMessage"]));
                                        $docValidation->setShowErrorMessage(self::boolean((string) $attrs["showErrorMessage"]));
                                        $docValidation->setErrorTitle((string) $attrs["errorTitle"]);
                                        $docValidation->setError((string) $attrs["error"]);
                                        $docValidation->setPromptTitle((string) $attrs["promptTitle"]);
                                        $docValidation->setPrompt((string) $attrs["prompt"]);
                                        $docValidation->setFormula1((string) $dataValidation->formula1->children('xm', true)->f);
                                        $docValidation->setFormula2((string) $dataValidation->formula2->children('xm', true)->f);
                                        $docSheet->setDataValidation($range, $docValidation);
                                    }
                                }
                            }
                    ...
                    /***SYNO start***/
                    $drawings[(string) $ele["Id"]] = (0 === strncmp($ele["Target"], "/", 1) ? substr($ele["Target"], 1) : self::dirAdd("$dir/$fileWorksheet", $ele["Target"]));
                    /***SYNO end***/
                                    }
                                }
                                    ...
                                    foreach ($xmlSheet->drawing as $drawing) {
                                        ...
                                        if ($relsDrawing && $relsDrawing->Relationship) {
                                            foreach ($relsDrawing->Relationship as $ele) {
                                                if ($ele["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/image") {
                            /***SYNO start***/
                            $images[(string) $ele["Id"]] = (0 === strncmp($ele["Target"], "/", 1) ? substr($ele["Target"], 1) : self::dirAdd($fileDrawing, $ele["Target"]));
                            /***SYNO end***/
                                            ...
                                            foreach ($xmlDrawing->oneCellAnchor as $oneCellAnchor) {
                                                if ($oneCellAnchor->pic->blipFill) {
                                                    ...
                                                    $chartDetails[$docSheet->getTitle().'!'.$thisChart] = array(
                                                        ...
                                                        'toOffsetX'         => $offsetX + $width,
                                                        'toOffsetY'         => $offsetY + $height,
                                        ...
                                        if ($xmlDrawing->twoCellAnchor) {
                                            foreach ($xmlDrawing->twoCellAnchor as $twoCellAnchor) {
                                                if ($twoCellAnchor->pic->blipFill) {
                                                    ...
                                                    $objDrawing->setAnchorType('twoCellAnchor');
                                                    $objDrawing->setEndCoordinates(PHPExcel_Cell::stringFromColumnIndex((string) $twoCellAnchor->to->col) . ($twoCellAnchor->to->row + 1));
                                                    $objDrawing->setEndOffsetX(PHPExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->to->colOff));
                                                    $objDrawing->setEndOffsetY(PHPExcel_Shared_Drawing::EMUToPixels($twoCellAnchor->to->rowOff));
                                        ...
                                        /***SYNO start***/
                                        if ($xmlDrawing->absoluteAnchor) {
                                            foreach ($xmlDrawing->absoluteAnchor as $absoluteAnchor) {
                                                if ($absoluteAnchor->pic->blipFill) {
                                                    $blip = $absoluteAnchor->pic->blipFill->children("http://schemas.openxmlformats.org/drawingml/2006/main")->blip;
                                                    $xfrm = $absoluteAnchor->pic->spPr->children("http://schemas.openxmlformats.org/drawingml/2006/main")->xfrm;
                                                    $objDrawing = new PHPExcel_Worksheet_Drawing;
                                                    $objDrawing->setName((string) self::getArrayItem($absoluteAnchor->pic->nvPicPr->cNvPr->attributes(), "name"));
                                                    $objDrawing->setDescription((string) self::getArrayItem($absoluteAnchor->pic->nvPicPr->cNvPr->attributes(), "descr"));
                                                    $objDrawing->setPath("zip://".PHPExcel_Shared_File::realpath($pFilename)."#" . $images[(string) self::getArrayItem($blip->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "embed")], false);
                                                    $objDrawing->setCoordinates('A1');
                                                    $objDrawing->setOffsetX(PHPExcel_Shared_Drawing::EMUToPixels(self::getArrayItem($absoluteAnchor->pos->attributes(), "x")));
                                                    $objDrawing->setOffsetY(PHPExcel_Shared_Drawing::EMUToPixels(self::getArrayItem($absoluteAnchor->pos->attributes(), "y")));
                                                    $objDrawing->setResizeProportional(false);
                                                    $objDrawing->setWidth(PHPExcel_Shared_Drawing::EMUToPixels(self::getArrayItem($absoluteAnchor->ext->attributes(), "cx")));
                                                    $objDrawing->setHeight(PHPExcel_Shared_Drawing::EMUToPixels(self::getArrayItem($absoluteAnchor->ext->attributes(), "cy")));
                                                    if ($xfrm) {
                                                        $objDrawing->setRotation(PHPExcel_Shared_Drawing::angleToDegrees(self::getArrayItem($xfrm->attributes(), "rot")));
                                                    }
                                                    $objDrawing->setWorksheet($docSheet);
                                                } else if (($this->includeCharts) && ($absoluteAnchor->graphicFrame)) {
                                                    //chart
                                                    $offsetX    = PHPExcel_Shared_Drawing::EMUToPixels(self::getArrayItem($absoluteAnchor->pos->attributes(), "x"));
                                                    $offsetY    = PHPExcel_Shared_Drawing::EMUToPixels(self::getArrayItem($absoluteAnchor->pos->attributes(), "y"));
                                                    $width      = PHPExcel_Shared_Drawing::EMUToPixels(self::getArrayItem($absoluteAnchor->ext->attributes(), "cx"));
                                                    $height     = PHPExcel_Shared_Drawing::EMUToPixels(self::getArrayItem($absoluteAnchor->ext->attributes(), "cy"));
                                                    $graphic    = $absoluteAnchor->graphicFrame->children("http://schemas.openxmlformats.org/drawingml/2006/main")->graphic;
                                                    $chartRef   = $graphic->graphicData->children("http://schemas.openxmlformats.org/drawingml/2006/chart")->chart;
                                                    $thisChart  = (string) $chartRef->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships");
                                                    $chartDetails[$docSheet->getTitle().'!'.$thisChart] = array(
                        ...
                        if ($xmlWorkbook->definedNames) {
                            foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
                                /*** SYNO start ***/
                                // Ignore built-in defined names
                                if (strpos((string)$definedName['name'], '_xlnm') === 0) {
                                    continue;
                                }

                                // Get referenced worksheet name & remove worksheet prefix
                                if (count($pairs = PHPExcel_Cell::splitRef((string)$definedName)) !== 1) {
                                    continue;
                                }
                                list($sheet, $value) = $pairs[0];

                                // Reject non-reference value
                                $value = strtoupper(str_replace('$', '', $value));
                                if (!preg_match("/^(\d+:\d+|[A-Z]+:[A-Z]+|[A-Z]+\d+(:[A-Z]+\d+)?)$/i", $value)) {
                                    continue;
                                }

                                // Get referenced worksheet, if not loaded faked with first sheet
                                if (($worksheet = $excel->getSheetByName($sheet)) === null) {
                                    $worksheet = $excel->getSheet();
                                }

                                if (isset($definedName['localSheetId'])) {
                                    $localSheetId = intval((string)$definedName['localSheetId']);
                                    $scope = (string)$xmlWorkbook->sheets->sheet[$localSheetId]['name'];
                                    $namedRange = new PHPExcel_NamedRange((string)$definedName['name'], $worksheet, $value, true, $scope);
                                    $namedRange->setLocalSheetId($localSheetId);
                                } else {
                                    $namedRange = new PHPExcel_NamedRange((string)$definedName['name'], $worksheet, $value);
                                }
                                $namedRange->setRawValue((string)$definedName);
                                $excel->addNamedRange($namedRange);
...
//                                    var_dump($chartDetails[$chartPositionRef]);
                                    ...
                                    /*** SYNO start ***/
                                    $objChart->setAnchorType($chartDetails[$chartPositionRef]['anchortype']);
                                    /*** SYNO end ***/
    ...
    /***SYNO start***/
    private static function readColor($color, $background = false, $indexstart = 0)
    /***SYNO end***/
    {
        if (isset($color["rgb"])) {
        ...
        /***SYNO start***/
        return PHPExcel_Style_Color::indexedColor($color["indexed"] - $indexstart, $background)->getARGB();
        /***SYNO end***/
...
//        }
        ...
        if (isset($style->fill)) {
            if ($style->fill->gradientFill) {
                ...
                /***SYNO start***/
                $docStyle->getFill()->getStartColor()->setARGB(self::readColor(self::getArrayItem($gradientFill->xpath("sml:stop[@position=0]"))->color, false, 0));
                $docStyle->getFill()->getEndColor()->setARGB(self::readColor(self::getArrayItem($gradientFill->xpath("sml:stop[@position=1]"))->color, false, 0));
                /***SYNO end***/
            ...
            /***SYNO start***/
            $docStyle->getFill()->getStartColor()->setARGB(self::readColor($style->fill->patternFill->fgColor, true, 0));
            /***SYNO end***/
                } else {
            ...
            /***SYNO start***/
            $docStyle->getFill()->getEndColor()->setARGB(self::readColor($style->fill->patternFill->bgColor, true, 0));
            /***SYNO end***/
                }
            }
        }
    ...
    private static function readBorder($docBorder, $eleBorder)
    {
        ...
        /***SYNO start***/
        $docBorder->getColor()->setARGB(self::readColor($eleBorder->color, false, 0));
        /***SYNO end***/
        }
    }
