...
//                echo '<br />';
                ...
                if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
                    ...
                    /***SYNO start***/
                    if (is_null($spContainer->getStartCoordinates())) {
                        continue;
                    }
                    /***SYNO end***/
...
//                            echo 'Picture Object<br />';
                            ...
                            if ($blip = $BSE->getBlip()) {
                                ...
                                /***SYNO start****/
                                $drawing->setImageResource($blip->getData());
                                /***SYNO end****/
...
//                    echo '<br />';
        ...
        foreach ($this->definedname as $definedName) {
            if ($definedName['isBuiltInName']) {
                ...
                if (count($pairs = PHPExcel_Cell::splitRef($definedName['formula'])) !== 1) {
                    continue;
                }
                ...
                if (($worksheet = $this->phpExcel->getSheetByName($sheet)) === null) {
                    $worksheet = $this->phpExcel->getSheet();
                }

                if ($definedName['scope'] > 0) {
                    $localSheetId = $definedName['scope'] - 1;
                    $scope = $this->sheets[$localSheetId]['name'];
                ...
                $namedRange->setRawValue($definedName['formula']);
                $this->phpExcel->addNamedRange($namedRange);
...
//            echo 'Note Text=', $noteText,'<br />';
            /***SYNO start***/
            $noteText = $this->decodeCodepage($noteText);
            /***SYNO end***/
            if ($extension) {
...
//                                                    ->setAuthor($author)
    ...
    private function readTextObject()
    {
        ...
        $realText = substr($text["recordData"], $text["spliceOffsets"][0] + 1, $textByte);
        // get 1 byte
        ...
        if (($is16Bit & 0x01) === 0) {
            $realText = PHPExcel_Shared_String::ConvertEncoding($realText, 'UTF-8', 'ISO-8859-1');
        } else {
            $realText = self::decodeCodepage($realText);
        }
